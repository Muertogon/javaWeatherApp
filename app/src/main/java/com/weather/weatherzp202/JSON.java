...
public class JSON {
    ...
    private static final String API_KEY = "4ebb48b17238d755e6e5120b459fde06";

    private RequestQueue queue;

    public JSON(@NonNull final Activity activity) {
        queue = Volley.newRequestQueue(activity.getApplicationContext());
    }

    public interface CurrentWeatherCallback {
        @MainThread
        void onCurrentWeather(@NonNull final Weather currentWeather);
                ...
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            //stuff for new entry
                            final double weatherPressure = currentWeatherJSONObject.getJSONObject("main").getDouble("pressure");
                            final double airHumidity = currentWeatherJSONObject.getJSONObject("main").getInt("humidity");
                            final double windSpeed = currentWeatherJSONObject.getJSONObject("wind").getDouble("speed");
                            //----------
                            final Weather currentWeather = new Weather(locationName, conditionId, conditionName, tempKelvin);
